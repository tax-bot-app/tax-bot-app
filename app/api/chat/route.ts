        ...
        const amountBias =
          lens === "amount"
            ? [
                ...
                "重要：金額の相談でも、初回回答（normal_llm）では『攻め/守り/危険』などの評価ラベルを付けない。ユーザー提示額に対して“良い/悪い”の結論を出さず、判断軸だけ整理する。",
                "重要：こちらで『○万〜○万』の数値レンジを勝手に作らない（ユーザーの金額を下限にして上限を作るのも禁止）。",
        "🥄ちょうど良いラインでは『この論点で何が見られるか』を2点だけ書く。具体的な中身（何が効くか）は、採用したQAの要点（qaKeyPointRule）に合わせて述べる。トピック固有の地雷を勝手に決め打ちしない。",
        "線引き（攻め/守り・どこまで・上限）を出すのは、ユーザーがLines要求をした時だけ（『攻め守りで』『さじかげんよろ』）。初回はそこまで踏み込まない。",
              ]
            : [];
