 ...
 // ===== QA採用スイッチ（自然会話のため）=====
      ...
      if (!answer) {
        ...
        const amountBias =
          lens === "amount"
            ? [
                 // ★方針転換：数値レンジを“創作”しない。ユーザー提示額を評価する。
                "重要：金額の相談でも、こちらで『○万〜○万』みたいな数値レンジを勝手に作らない（ユーザーの金額を下限にして上限を作るのも禁止）。",
                "🥄ちょうど良いラインでは、①ユーザー提示の金額が『守り/ちょい攻め/攻め（要説明）』のどれかをまず一言で評価する。②次に、その評価が動く条件（頻度・相手・目的・メモ・私用混入）を2点だけ出す。",
                "数値を書くのは『制度上の明確な基準』がある時だけ（例：1万円基準）。それ以外は数値を増やさず、超える場合の“運用条件”で攻め守りを分ける。",
              ]
            : [];
