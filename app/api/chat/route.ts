...
function buildOutputRules(params: { allowAttackDefenseDetail: boolean }): string[] {
  ...
  const detailRule = allowAttackDefenseDetail
    ? "ユーザーが深掘りを求めた場合は、**必ず** 見出し付きで『🍚攻め』『🧂守り』の詳細ブロックを出す。『🥄先に言うと』や要点の繰り返しは禁止。"
    : "初回回答では『🍚攻め』『🧂守り』は出さない。判断軸として『🥄先に言うと』と要点の説明だけに限定する。";

  return [
    "Markdownの見出し（##、###など）は使わない。",
    "見出しラベルは固定：『🥄先に言うと』『✅要点』『⚠️注意』。", // ← 🔎確認を削除
    "初回は『🥄先に言うと』→『✅要点』の順で書く。",
    detailRule,
...
function catchphraseFor(dialect: Dialect, stance: Stance): string {
  ...
  if (dialect === "standard" && stance === "sanbo") {
    return "とはいえ、税務の判断は白黒だけではありません。🍚**『攻め守りで』**や**『さじかげんよろしく』**と言っていただければ、実務的なラインをお出しします。";
  }
  if (dialect === "kansai" && stance === "zubatto") {
    return "とはいえ、税務は一発正解ちゃう。🍚**『攻め守りで』**とか**『さじかげんよろ』**って言うたら、実務的なライン出すで。";
  }
  return "とはいえ、税務の世界は答えはひとつじゃないよ。🍚**『攻め守りで』**とか**『さじかげんよろ』**って言ってくれたら、実務的なライン出すよ。";
}
...
function stripInternalLeaks(text: string): string {
  ...
  s = out.join("\n").trim();
  if (!s)
    s = "🥄先に言うと：**一般論で整理する**。必要なら条件を揃えて深掘りする。";
  return s;
}
...
// --- END prev/context fetch ---
      ...
      // ルール：直前が税務っぽい時だけ発火（雑談誤爆を潰す）
      const prevForBorrow = String(prevUserMessageRaw || prevUserMessage || "");
      const prevLooksTax =
        /税|経費|損金|消費税|仕訳|領収|請求|売上|給与|交際費|日当|外注|源泉|調査|否認|法人|個人|申告|決算|勘定科目|証憑/.test(
          prevForBorrow
        );
...
// 金額レンジの続きは amount Lines を優先する（回答のlensとは責務を分ける）
 ...
 // ===== QA採用スイッチ（自然会話のため）=====
   ...
   mode === "demo" ? ["【ルール】会話と矛盾しない。確認は原則1つ。"] : await buildConversationContext({ db, convId });
        ...
        const amountBias =
          lens === "amount" && mode === "demo"
            ? [
                "重要：提示された金額に正面から反応し、有利に整理できる可能性があるなら、条件付きで先に示す。無条件に肯定はしない。",
                "重要：提示額を実現するための主要条件を2〜4点、具体的に示す。規程・証拠・実態・継続運用など、今回の論点に合うものへ絞る。",
                "重要：こちらで新しい数値レンジや上限を創作しない。ユーザーが示した金額について、成立可能性と判断軸を整理する。",
              ]
            : lens === "amount"
            ? [
...
const warmCloseRule = wantWarmClose
  ? mode === "demo"
    ? [
        "【デモの締め】最後に、今回の結論を御社向けに具体化するための確認質問を1〜3行で必ず書く。各行の行頭に（〆）を付ける。",
        "【デモの締め】質問は、会社規模・金額・対象者・頻度・規程・証拠・現在の運用のうち、結論が動く重要な前提へ絞る。",
        "【デモの締め】一般的な『掘り下げますか？』では終えず、ユーザーがそのまま回答できる具体的な聞き方にする。",
      ].join("\n")
    : closeMode === "followup"
    ? [
        "【締め】最後に1〜3行の締めを書く。各行の行頭に（〆）を付ける（目印。ユーザー表示では消える）。",
        "【締め】記号やラベル（🔎・👉など）は使わない。箇条書き禁止。括弧（）も使わない。",
        "【締め】同じ言い回しを毎回使わない。直前の文脈に沿って言い回しを変える（コピペ禁止）。",
        "【締め】質問は最大2つまで。『A？それともB？』の二択は1つの質問として数える。二択を2回やらない。",
        "【締め】続きの流れでは交通整理はしない。代わりに、深掘りできる論点を1〜2個だけ提示して、軽い問いかけで次に繋げる。",
      ].join("\n")
    : closeMode === "light"
      ? [
          "【締め】最後に1〜2行で、空気を柔らかくする一言だけ添える（目印として各行の先頭に（〆）ユーザー表示では消える）。",
"【締め】記号やラベル（🔎・👉など）は使わない。箇条書き禁止。括弧（）も使わない。",
"【締め】同じ言い回しを毎回使わない。直前の文脈に沿って言い回しを変える（コピペ禁止）。",
"【締め】内容は『前提が増えたら結論が動く』ことを匂わせる程度に留める。新しい論点を増やさない。",
"【締め】質問形で終えない（確認質問ゼロ）。ユーザーが次を言いやすい余白で止める。",
        ].join("\n")
      : [
          // default = amb
           "【締め】最後に1〜3行の締めを書く。各行の行頭に（〆）を付ける（目印。ユーザー表示では消える）。",
          "【締め】記号やラベル（🔎・👉など）は使わない。箇条書き禁止。括弧（）も使わない。",
          "【締め】同じ言い回しを毎回使わない。直前の文脈に沿って言い回しを変える（コピペ禁止）。",
          "【締め】内容は『次に決めるべき前提の分岐』を1つだけ提示し、質問は最大1つまで（詰問禁止）。",
"【締め】二択にするなら1回だけ（A？それともB？）。二択を2回やらない。",
"【締め】“聞き返し感”を出さず、会話の延長で自然に聞く。長い前置き・一般論の繰り返しは禁止。",
        ].join("\n")
  : null;
...
const borrowAssumeTaxRule = borrowingContext
  ? [
      "【前提（内規）】直前文脈を参照して推測で答える場合でも、必ず税務・会計・経営の前提で回答する。",
      "【前提（内規）】雑談・一般論・商品比較など別ジャンルへの脱線は禁止。税務の論点整理に固定する。",
      "【前提（内規）】不足情報がある場合は、結論を崩さずに確認質問は最大1つまで（聞き返し連打禁止）。",
      "【注意】この前提をユーザー向けの宣言文として書かない。",
    ].join("\n")
  : null;
...
// デモも判断の価値が伝わるところまで出し、有料版は自社向けの実行設計で差を付ける。
        const demoAnswerRules: string[] =
          mode === "demo"
            ? [
                "【デモ回答】結論の入口だけで止めず、ユーザーに有利な可能性がある場合は条件付きで先に示す。",
                "【デモ回答】『こうすれば成立しやすい』という主要条件を2〜4点、具体的に示す。",
                "【デモ回答】税務調査で問題になりやすい点を1〜2点示す。",
                "【デモ回答】最後の会社別確認質問は省略しない。",
              ]
            : [];
