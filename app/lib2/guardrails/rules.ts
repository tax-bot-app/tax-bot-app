...
export const RULES: Rule[] = [
  ...
  // =========================
  {
    ...
    patterns: [
      ...
      /領収書.*(作|つく)/i,
      /(請求書|領収書).*(偽造|改ざん|改竄)/i,
      /(領収書|請求書).*(日付|日付だけ).*(ずら|ズラ|変更|変え)/i,   // ★追加
    /(日付).*(ずら|ズラ|変更|変え).*(領収書|請求書)/i,           // ★追加
    ],
  },
  {
  level: 1,
  ...
  patterns: [
    ...
    // ★追加：帳簿に入れない/計上しない/載せない（口語も含む）
    /(現金売上|売上|入金).*(帳簿|帳簿に|帳簿へ|会計|帳票).*(入れ(ない|ん|へん|ず)|載せ(ない|ん|へん)|計上し(ない|ん|へん))/i,
    /(帳簿|会計).*(入れ(ない|ん|へん)|載せ(ない|ん|へん)|計上し(ない|ん|へん)).*(現金売上|売上|入金)/i,
    /(売上|現金売上).*(抜|除外|計上しない|計上せん|帳簿に入れない|帳簿に入れん|帳簿に入れへん)/i,
